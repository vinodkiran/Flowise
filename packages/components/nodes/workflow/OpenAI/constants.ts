...
export const getPrompts = () => {
    const returnArray: INodeOptionsValue[] = []
    const sortedPrompts = JSON.parse(JSON.stringify(prompts.sort((a, b) => (a.act < b.act ? -1 : a.act > b.act ? 1 : 0))))
    sortedPrompts.unshift({
        act: 'Anything',
        prompt: 'How are you today?'
    })
    for (let i = 0; i < sortedPrompts.length; i += 1) {
        returnArray.push({
            label: sortedPrompts[i].act,
            name: sortedPrompts[i].act.replace(/\s/g, '')
        })
    }
    return returnArray
}
...
export const setPrompt = () => {
    const returnArray: INodeParams[] = []
    const clonedPrompts = JSON.parse(JSON.stringify(prompts))
    clonedPrompts.unshift({
        act: 'Anything',
        prompt: 'How are you today?'
    })
    for (let i = 0; i < clonedPrompts.length; i += 1) {
        const obj: INodeParams = {
            label: 'Text Prompt',
            name: `${clonedPrompts[i].act.replace(/\s/g, '')}Prompt`,
            type: 'string',
            rows: 5,
            default: `${clonedPrompts[i].prompt}`,
            description: `The prompt for chatpgt ${clonedPrompts[i].act}`,
            show: {
                'inputParameters.chatGPTPersona': [clonedPrompts[i].act.replace(/\s/g, '')]
            }
        }
