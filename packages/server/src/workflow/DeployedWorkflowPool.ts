import { INodeData, INodeExecutionData, IWebhookNodeExecutionData } from 'flowise-components'
import {
...
import { constructGraphs, getStartingNode } from '../utils/workflow.utils'
...
export class DeployedWorkflowPool {
            ...
            if (workflow.deployed) {
                try {
                    ...
                    if (faultyNodeLabels.length) {
                        ...
                        AppDataSource.getRepository(WorkFlow).merge(workflow, updateWorkflow)
     ...
     * @param activeTestTriggerPool
     * @param activeTestWebhookPool
     */
    async add(
        ...
        for (let i = 0; i < startingNodeIds.length; i += 1) {
            ...
            if (startNode && startNode.data && startNode.data.type === 'trigger') {
                ...
                await decryptNodeCredentials(startNode.data, this.AppDataSource)
                ...
                const foundWebhook = await this.AppDataSource.getRepository(Webhook).findOneBy(newBody)
                ...
                if (foundWebhook && foundWebhook.webhookId && process.env.ENABLE_TUNNEL === 'true') {
                    ...
                    await this.AppDataSource.getRepository(Webhook).delete({
                        id: foundWebhook.id
                    })
                    await decryptNodeCredentials(startNode.data, this.AppDataSource)
                    ...
                    const webhook = this.AppDataSource.getRepository(Webhook).create(newWebhook)
                    await this.AppDataSource.getRepository(Webhook).save(webhook)
                }
    ...
    async remove(
        ...
        for (let i = 0; i < startingNodeIds.length; i += 1) {
            ...
            if (startNode && startNode.data && startNode.data.type === 'trigger') {
                ...
                const webhook = await this.AppDataSource.getRepository(Webhook).findOneBy(query)
                ...
                await this.AppDataSource.getRepository(Webhook).delete(query)
            }
    ...
    async removeAll(componentNodes: IComponentNodesPool) {
        ...
        const workflows: WorkFlow[] = await this.AppDataSource.getRepository(WorkFlow).findBy({
            shortId: In(deployedWorkflowShortIds)
        })
    ...
    async addExecution(workflowShortId: string, workflowExecutedData: IWorkflowExecutedData[], controller: AbortController) {
        ...
        return await this.AppDataSource.getRepository(Execution).save(execution)
    }
    ...
    async updateExecution(
        ...
        await this.AppDataSource.getRepository(Execution).save(execution)
    }
    ...
    async terminateExecutions(workflowShortId: string) {
        const executions: Execution[] = await this.AppDataSource.getRepository(Execution).findBy({
            workflowShortId,
            state: 'INPROGRESS' as ExecutionState
        })
