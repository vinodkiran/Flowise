...
const AddEditDatasetRowDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
    ...
    const addNewDatasetRow = async () => {
        try {
            ...
            enqueueSnackbar({
                message: `Failed to add new row in the Dataset: ${errorData}`,
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
    ...
    const saveDatasetRow = async () => {
        try {
            ...
            enqueueSnackbar({
                message: `Failed to save Dataset Row: ${errorData}`,
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
            ...
            <DialogTitle sx={{ fontSize: '1rem' }} id='alert-dialog-title'>
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    {dialogProps.type === 'ADD'
                        ? 'Add Item to [' + datasetName + '] Dataset'
                        : 'Edit Item in Dataset [' + datasetName + ']'}
                </div>
            </DialogTitle>
            <DialogContent>
                <Box sx={{ p: 2 }}>
